...
describe('Provider Controller - Email Verification Gate', () => {
  let mockResponse: Partial<Response>;
  let responseJson: jest.Mock;
  let responseStatus: jest.Mock;
  ...
  it('returns 403 when user has not verified their email', async () => {
    ...
    await createProvider(req, mockResponse as Response);

    expect(responseStatus).toHaveBeenCalledWith(403);
    expect(responseJson).toHaveBeenCalledWith({
      message: 'Please verify your email address before submitting your profile.',
    });
  });

  it('allows profile creation when user has verified their email', async () => {
